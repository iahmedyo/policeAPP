    ...
    public partial class frmLogs : Form
    {
        ...
        public void fillCombo()
        {
            try
            {
                ...
                cc.da.SelectCommand = new SqlCommand("SELECT distinct login FROM [User]", cc.con);
      ...
      private void btnGetData_Click(object sender, EventArgs e)
      {
          ...
          try
          {
              cc.con = new SqlConnection(cs.DBConn());
              cc.con.Open();
              cc.cmd = new SqlCommand("SELECT RTRIM(username),RTRIM(dateoperation),RTRIM(operation) from logs where dateoperation between @date1 and @date2 order by dateoperation", cc.con);
              cc.cmd.Parameters.Add("@date1", SqlDbType.DateTime, 30, "dateoperation").Value = dtpDateFrom.Value.Date;
              cc.cmd.Parameters.Add("@date2", SqlDbType.DateTime, 30, "dateoperation").Value = dtpDateTo.Value;
      ...
      private void GetData()
      {
          try
          {
              cc.con = new SqlConnection(cs.DBConn());
              cc.con.Open();
              cc.cmd = new SqlCommand("SELECT RTRIM(username),RTRIM(dateoperation),RTRIM(operation) from Logs order by dateoperation", cc.con);
              cc.rdr = cc.cmd.ExecuteReader(CommandBehavior.CloseConnection);
              dgw.Rows.Clear();
              while ((cc.rdr.Read() == true))
              {
      ...
      private void cmbUserID_SelectedIndexChanged(object sender, EventArgs e)
      {
          ...
          try
          {
              cc.con = new SqlConnection(cs.DBConn());
              cc.con.Open();
              cc.cmd = new SqlCommand("SELECT RTRIM(username),RTRIM(dateoperation),RTRIM(operation) from Logs where username='" + cmbUserID.Text + "' order by date", cc.con);
              cc.rdr = cc.cmd.ExecuteReader(CommandBehavior.CloseConnection);
              dgw.Rows.Clear();
              while (cc.rdr.Read() == true)
              {
